...
foreach($_GET as $key => $value)
{
	...
	if(substr($key,0,3) =='arr')
	{
		...
		$clauseres ="AND p.date_arrivee >= '$hour_arrive' AND p.date_depart <= '$hour_depart'";
	}
...
$connexion_pdo = $pdo->query("SELECT * FROM produit p, salle s WHERE p.id_salle = s.id_salle $whereclause $clauseville  $clausecapacite $clauseprix $clauseres AND p.date_arrivee > NOW() AND p.etat='libre' AND p.quantite='1' ORDER BY p.prix ASC");
